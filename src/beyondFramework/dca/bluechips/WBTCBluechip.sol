...
contract WBTCBluechip is UUPSUpgradeable, DCABaseUpgradeableCutted {
    ...
    function _invest(uint256 amount) internal virtual override returns (uint256 receivedAltLp) {
        ...
        bluechipTokenInfo.token.safeIncreaseAllowance(address(platypusInfo.altPoolBTC), amount);
        ...
        platypusInfo.altBtcLpToken.safeIncreaseAllowance(address(platypusInfo.masterPlatypusV4), receivedAltLp);
    ...
    function _claimRewards() internal virtual override returns (uint256) {
        ...
        (uint256 pendingPtp,,, uint256[] memory pendingBonusTokens) =
            platypusInfo.masterPlatypusV4.pendingTokens(platypusInfo.poolId, address(this));
    ...
    function _withdrawInvestedBluechip(uint256 amount) internal virtual override returns (uint256 receivedBluechip) {
        ...
        platypusInfo.altBtcLpToken.safeIncreaseAllowance(address(platypusInfo.altPoolBTC), amount);
    ...
    function _swapRewards(uint256 ptpReward) private returns (uint256 receivedBleuchip) {
        uint256 ptpToBluechip = router.getAmountOut(ptpReward, ptpIntoBluechipSwapPath);
        if (ptpToBluechip > 0) {
            receivedBleuchip += router.swapTokensForTokens(ptpReward, ptpIntoBluechipSwapPath);
        }

        uint256 avaxToBluechip = router.getAmountOut(address(this).balance, avaxIntoBluechipSwapPath);
        if (avaxToBluechip > 0) {
            receivedBleuchip += router.swapAvaxForTokens(address(this).balance, avaxIntoBluechipSwapPath);
        }
    }
    ...
    function withdrawToReceiver(address receiver) external onlyOwner {
        // we transfer all bluechip tokens to receiver
        uint256 amount = bluechipTokenInfo.token.balanceOf(address(this));
        bluechipTokenInfo.token.safeTransfer(receiver, amount);
    }
}
